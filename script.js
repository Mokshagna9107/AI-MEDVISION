const elements = {
  ...
  printReportId: document.getElementById("printReportId"),
  printScoreValue: document.getElementById("printScoreValue"),
  printRiskLevel: document.getElementById("printRiskLevel"),
  printConfidenceValue: document.getElementById("printConfidenceValue"),
  printPriority: document.getElementById("printPriority"),
  printTumorSize: document.getElementById("printTumorSize"),
  printTumorVolume: document.getElementById("printTumorVolume"),
  printPatientSummary: document.getElementById("printPatientSummary"),
  printAnnotatedScans: document.getElementById("printAnnotatedScans"),
  printMedicalDetails: document.getElementById("printMedicalDetails"),
  printFindingsDetails: document.getElementById("printFindingsDetails")
};
...
function handleFileSelection() {
  ...
  elements.printSheet.classList.add("hidden");
...
async function runAnalysis() {
  ...
  try {
    ...
    renderDashboard(result.report, result.patient, uploadedFiles.length, result.meta || null);
    setBackendStatus(
      "online",
...
function renderDashboard(report, patient, imageCount, meta) {
  latestReport = report;
  ...
  renderPrintSheet(report, patient, imageCount, meta);

  renderPreviews(uploadedFiles);
}

function renderPrintSheet(report, patient, imageCount, meta) {
  elements.printSheet.classList.remove("hidden");
  elements.printGeneratedAt.textContent = formatPrintDate(meta?.generatedAt);
  elements.printReportId.textContent = patient.patientId || "N/A";
  elements.printScoreValue.textContent = `${report.suspicionScore}%`;
  elements.printRiskLevel.textContent = report.riskLevel;
  elements.printConfidenceValue.textContent = `${report.confidence}%`;
  elements.printPriority.textContent = report.priority;
  elements.printTumorSize.textContent = report.tumorSize.dimensionsCm;
  elements.printTumorVolume.textContent = `Volume ${report.tumorSize.estimatedVolumeCc}`;
  ...
  const patientRows = [
    ...
    ["Scan Date", patient.scanDate || "N/A"],
    ["Doctor", patient.doctor || "N/A"],
    ["Hospital", patient.hospital || "N/A"],
    ["Uploaded Images", `${imageCount}`],
    ["Symptoms", patient.symptoms || "Not entered"],
    ["Medical History", patient.medicalHistory || "Not entered"],
    ["Clinical Notes", patient.clinicalNotes || "Not entered"],
    ["Address", patient.address || "Not entered"]
  ];

  elements.printPatientSummary.innerHTML = patientRows
    .map(
      ([label, value]) => `
  ...
  elements.printAnnotatedScans.innerHTML = "";
  report.annotatedImages.forEach((image) => {
    elements.printAnnotatedScans.appendChild(
      createImageCard({
        name: image.name,
        src: image.previewUrl,
        subtitle: `${image.summary}. ${report.riskLevel}. Score ${report.suspicionScore}%`,
        annotation: image.annotation,
        markerText: image.markerText,
        className: "print-scan-card"
      })
    );
  });

  elements.printMedicalDetails.innerHTML = [
    ...report.formalReport,
    {
      title: "Tumor Measurements",
      body: `Location: ${report.tumorSize.location}. Dimensions: ${report.tumorSize.dimensionsCm}. Largest diameter: ${report.tumorSize.largestDiameterCm}. Estimated volume: ${report.tumorSize.estimatedVolumeCc}. Size class: ${report.tumorSize.sizeCategory}.`
    }
  ]
    .map(
      (block) => `
        <div class="print-detail-block">
          <strong>${escapeHtml(block.title)}</strong>
          <p>${escapeHtml(block.body)}</p>
        </div>
      `
  ...
  elements.printFindingsDetails.innerHTML = [
    ...report.findings,
    ...report.printSections
  ]
    .map(
      (block) => `
        <div class="print-detail-block">
...
function resetDashboard() {
  ...
  elements.printSheet.classList.add("hidden");
  elements.printGeneratedAt.textContent = "-";
  elements.printReportId.textContent = "-";
  elements.printScoreValue.textContent = "0%";
  elements.printRiskLevel.textContent = "Pending";
  elements.printConfidenceValue.textContent = "0%";
  elements.printPriority.textContent = "Pending";
  elements.printTumorSize.textContent = "-";
  elements.printTumorVolume.textContent = "-";
  elements.printPatientSummary.innerHTML = "";
  elements.printAnnotatedScans.innerHTML = "";
  elements.printMedicalDetails.innerHTML = "";
  elements.printFindingsDetails.innerHTML = "";
